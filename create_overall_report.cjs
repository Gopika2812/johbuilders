...
const overallReportContent = `import React, { useState, useEffect } from 'react';
  ...
  const handleExport = () => {
    const exportData = filteredFlows.map((flow, index) => {
      ...
      return {
        ...
        'Project Type': flow.project?.projectType || 'N/A',
                  ...
                  filteredFlows.map((flow, idx) => {
                    ...
                    return (
                      <tr key={flow._id} className="hover:bg-gray-50/50 transition">
                        ...
                        <td className="p-3 text-gray-600 font-medium">{flow.project?.projectType || 'N/A'}</td>
...
fs.writeFileSync(path.join('client', 'src', 'pages', 'OverallReport.jsx'), overallReportContent);

// Add to App.jsx
let appJsx = fs.readFileSync(path.join('client', 'src', 'App.jsx'), 'utf8');
if (!appJsx.includes('import OverallReport')) {
...
if (!sidebarJsx.includes('/crd-flow/overall-report')) {
  const sidebarTarget = \`              <Link
                ...
                className={\\\`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs transition \\\${
                  isActive('/crd-flow/collection-report')
  ...
  const sidebarReplacement = \`              <Link
              ...
              >
                <span className={sidebarOpen ? "block truncate" : "hidden"}>Overall Report</span>
              </Link>\`;
...
console.log('Overall report created and integrated');
