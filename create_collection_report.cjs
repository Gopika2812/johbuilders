...
const collectionReportContent = `import React, { useState, useEffect } from 'react';
...
fs.writeFileSync(path.join('client', 'src', 'pages', 'CollectionReport.jsx'), collectionReportContent);

// Add to App.jsx
let appJsx = fs.readFileSync(path.join('client', 'src', 'App.jsx'), 'utf8');
if (!appJsx.includes('import CollectionReport')) {
  ...
  fs.writeFileSync(path.join('client', 'src', 'App.jsx'), appJsx);
}

// Add to Sidebar.jsx
let sidebarJsx = fs.readFileSync(path.join('client', 'src', 'components', 'Sidebar.jsx'), 'utf8');
if (!sidebarJsx.includes('/crd-flow/collection-report')) {
  const sidebarTarget = \`              <Link
                to="/crd-flow/bank-loan-history"
                className={\\\`flex items-center gap-3 px-4 py-2 text-xs font-semibold rounded-xl transition-all duration-200 \\\${
                  location.pathname === '/crd-flow/bank-loan-history'
  ...
  const sidebarReplacement = \`              <Link
              ...
              <Link
                to="/crd-flow/collection-report"
                className={\\\`flex items-center gap-3 px-4 py-2 text-xs font-semibold rounded-xl transition-all duration-200 \\\${
                  location.pathname === '/crd-flow/collection-report'
                    ? 'bg-[#0e623a] text-white shadow-md'
                    : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
                }\\\`}
              >
                <Layers className="w-4 h-4" />
                <span className={sidebarOpen ? "block truncate" : "hidden"}>Collection Report</span>
              </Link>\`;

  if (sidebarJsx.includes(sidebarTarget)) {
    sidebarJsx = sidebarJsx.replace(sidebarTarget, sidebarReplacement);
    fs.writeFileSync(path.join('client', 'src', 'components', 'Sidebar.jsx'), sidebarJsx);
...
console.log('Collection report created and integrated');
