...
const SettingsPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {activeTab === 'general' ? (
        <div className="bg-white rounded-b-3xl shadow-sm border border-gray-100 overflow-hidden animate-fadeIn">
          ...
          <form onSubmit={handleSaveGeneral} className="p-8 space-y-8">
            ...
            <div className="space-y-4 text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Display Name</label>
                  <input
                    type="text"
                    disabled
                    value={user?.name || ''}
                ...
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Active Email</label>
                  <input
                    type="email"
                    disabled
                    value={user?.email || ''}
              ...
              <span className="text-[10px] text-gray-400 flex items-center gap-1.5 mt-1 font-light">
            ...
            <div className="space-y-4 text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Company Name</label>
                  <input
                    type="text"
                    value={companyName}
                    onChange={(e) => setCompanyName(e.target.value)}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0e623a] transition"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Measuring Unit System</label>
                  <select
                    value={measureUnit}
                ...
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Default Currency</label>
                  <select
                    value={currency}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left animate-fadeIn">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto border border-gray-200 rounded-2xl shadow-inner bg-white">
                            ...
                            <td className="p-4 font-bold text-gray-850">
                              {isEditing ? (
                                <input
                                  ...
                                  className="px-2 py-1 bg-gray-50 border rounded focus:ring-1 focus:ring-[#0e623a] text-xs font-semibold"
                                />
                              ) : (
                                group.name
                              )}
                              ...
                              {isEditing ? (
                                <div className="space-y-1">
                                  <div className="flex items-center justify-between gap-1 mb-1">
                                    <input
                                      ...
                                      className="px-2 py-0.5 bg-gray-50 border rounded text-[10px] w-full focus:outline-none focus:ring-1 focus:ring-[#0e623a]"
                                    />
                                    <button
                                      type="button"
                                      onClick={() => {
                                      ...
                                      className="text-[9px] font-bold text-[#0e623a] hover:underline whitespace-nowrap px-1 bg-emerald-50 rounded"
                                    >
                                      Toggle All
                                    </button>
                                  </div>
                                  <div className="max-h-24 overflow-y-auto border border-gray-200 rounded p-1 space-y-1">
                                ...
                                <div className="flex flex-wrap gap-1.5 max-w-[240px]">
                                  {group.sources?.map(src => (
                                    <span key={src} className="px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded text-[9px] font-semibold">
                                      {src}
                                    </span>
                                  ))}
                                </div>
                                ...
                                {isEditing ? (
                                  <>
                                    ...
                                    <button
                                      ...
                                      className="px-2 py-1 bg-gray-150 text-gray-600 rounded text-[10px] font-bold hover:bg-gray-200"
                                    >
                                      Cancel
                                    </button>
                                  </>
                                  ...
                                  <>
                                    <button
                                      ...
                                      className="p-1 text-gray-400 hover:text-[#0e623a] hover:bg-gray-100 rounded"
                                    >
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-sm space-y-4 h-fit">
              <div className="border-b pb-3">
                <h3 className="text-base font-bold text-gray-800 flex items-center gap-2">
                  <Plus className="w-5 h-5 text-[#0e623a]" />
                  <span>Create Lead Group</span>
                </h3>
                <p className="text-[10px] text-gray-500 mt-1">Bundle specific channels and define dynamic budgets.</p>
              </div>

              <form onSubmit={handleCreateGroup} className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1.5">Group Name</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#0e623a] text-xs font-semibold"
                  />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Select Lead Sources</label>
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 mb-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#0e623a] text-xs font-semibold"
                  />
                  <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto border border-gray-150 rounded-xl p-3 bg-gray-50/50">
                    {SOURCE_TYPES.filter(src => src.toLowerCase().includes(sourceSearch.toLowerCase())).map(src => (
                      <label key={src} className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={selectedSources.includes(src)}
                          onChange={() => toggleSourceSelection(src)}
                          className="rounded text-[#0e623a] focus:ring-[#0e623a] border-gray-300"
                        />
                        <span>{src}</span>
                      </label>
                    ))}
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 bg-[#0e623a] text-white rounded-xl text-xs font-bold hover:bg-[#0b4d2d] transition"
                >
                  Create & Save Group
                </button>
              </form>
