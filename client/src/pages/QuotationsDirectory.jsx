...
const QuotationsDirectory = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-gray-150 p-6 rounded-3xl shadow-sm">
        <div>
          <h2 className="text-xl font-black text-gray-800 flex items-center gap-2">
            <FileText className="w-6 h-6 text-[#0e623a]" />
            <span>Quotation Records Directory</span>
          </h2>
          <p className="text-gray-400 text-xs mt-1">Review, generate, edit and print client valuation estimates</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-150 p-4 rounded-3xl shadow-sm flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#0e623a] text-xs font-semibold text-gray-700 placeholder-gray-400"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="p-12 text-center text-xs text-gray-400">Loading quotation database records...</div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {filteredQuotations.map(q => (
                  <tr key={q._id} className="hover:bg-gray-50/50 transition duration-150">
                    {/* Customer */}
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-[10px] text-gray-400 mt-0.5">{q.project?.name || ''}</div>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap gap-1">
                        {q.selectedUnits.map(unit => (
                          <span key={unit} className="px-2 py-0.5 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded text-[10px] font-bold">
                            {unit}
                          </span>
                        ))}
                      </div>
                    ...
                    {/* Total Value */}
                    <td className="p-4">
                      <div className="font-extrabold text-gray-800">Rs. {q.totalValue.toLocaleString()}</div>
                      <div className="text-[9px] text-gray-400 mt-0.5">Rate: Rs. {q.pricePerSqFt}/Sq.Ft</div>
                    </td>

                    {/* Prepared By */}
                    <td className="p-4">
                      <div className="font-semibold text-gray-700">{q.createdBy?.name || 'System'}</div>
                      <div className="text-[10px] text-gray-400">{q.createdBy?.role || 'User'}</div>
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        <Link
                          to={`/quotations/${q._id}`}
                          className="p-2 text-gray-500 hover:text-[#0e623a] hover:bg-[#0e623a]/5 rounded-xl transition"
