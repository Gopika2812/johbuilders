...
const LeadsDirectory = () => {
  ...
  return (
    <div className="space-y-1 w-full max-w-full pb-0">
      ...
      <div className="shrink-0 bg-white p-1.5 sm:p-2 border border-black-150 shadow-xs rounded-xl space-y-1">
      ...
      <div className="bg-white border border-black-150 shadow-xs rounded-xl overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead className="sticky top-0 z-20 shadow-xs">
              <tr className="bg-black-50 border-b border-black-150 text-[10.5px] font-bold text-black-600 uppercase tracking-wider">
                {hasColumnPermission('leads', 'sno') && <th className="px-2 py-1.5 w-10 text-center">S.No</th>}
                {hasColumnPermission('leads', 'date') && <th className="px-2 py-1.5">Date</th>}
                {hasColumnPermission('leads', 'customerName') && <th className="px-2 py-1.5">Customer Name</th>}
                {hasColumnPermission('leads', 'phoneNumber') && <th className="px-2 py-1.5">Phone Number</th>}
                {hasColumnPermission('leads', 'sourceDetails') && <th className="px-2 py-1.5">Source Details</th>}
                {hasColumnPermission('leads', 'project') && <th className="px-2 py-1.5">Project</th>}
                {hasColumnPermission('leads', 'category') && <th className="px-2 py-1.5 text-center">Category</th>}
                {hasColumnPermission('leads', 'assignedBy') && <th className="px-2 py-1.5">Assigned By</th>}
                {hasColumnPermission('leads', 'assignedTo') && <th className="px-2 py-1.5">Assigned To</th>}
                {hasColumnPermission('leads', 'leadStatus') && <th className="px-2 py-1.5">Lead Status</th>}
                {hasColumnPermission('leads', 'nextFollowup') && <th className="px-2 py-1.5 text-center">Next Followup</th>}
                <th className="px-2 py-1.5 text-center">WhatsApp</th>
                {hasColumnPermission('leads', 'actions') && <th className="px-2 py-1.5 text-center">Actions</th>}
              </tr>
            </thead>
                ...
                return (
                  ...
                  >
                    ...
                    {hasColumnPermission('leads', 'phoneNumber') && (
                      <td className="px-2 py-1.5 sm:py-2 border-b border-black-100">
                        <div className="flex items-center gap-1 font-semibold text-[11px] text-black-500 leading-tight">
                          <Phone className="w-3 h-3 text-black-300 shrink-0" />
                          <span>{lead.phone}</span>
                        </div>
                        {lead.alternativePhone && (
                          <div className="flex items-center gap-1 font-medium text-[10px] text-black-400 leading-tight mt-0.5">
        ...
        {/* Pagination Controls */}
        <div className="shrink-0 flex flex-col sm:flex-row items-center justify-between px-3 py-2 sm:py-2.5 border-t border-black-150 gap-2 bg-black-50/50 rounded-b-xl">
